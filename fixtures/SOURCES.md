...
- `zig build security-check` (also runs as part of `zig build test`) scans the tracked fixture, source, example, benchmark, tool, and integration roots, plus selected public root files, for private-key PEM markers and other guarded material. This is a **build-time** tool under `tools/`; it is not linked into the z-oci library binary.
